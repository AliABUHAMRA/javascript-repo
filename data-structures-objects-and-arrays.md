...
يمكنك تغيير خصائصها ، مما يتسبب في وجود محتوى مختلف لقيمة كائن واحد في أوقات مختلفة.

عندما يكون لدينا رقمان ، على سبيل المثال 120 و 120 ، يمكننا اعتبار كلاهما نفس الرقم تمامًا ، بغض النظر عما إذا كانا مرتبطين بنفس الجسم  أم لا. لكن في الكائنات ، هناك فرق بين وجود مرجعين لنفس الكائن وبين وجود كائنين مختلفين لهما خصائص متشابهة. لاحظ الرموز التالية:
...
عامل التشغيل ==  JavaScript ، عند استخدامه لمقارنة الكائنات لهما نفس القيمه ، فإنه يرجع true فقط إذا كان كلا الكائنين بنفس القيمة تمامًا. عند المقارنة بين كائنات مختلفة ، حتى عندما يكون محتواها واحدًا ، ينتج عن ذلك false. في JavaScript لتاكد انظر للمثال الاعلى ، لا توجد عملية مقارنة "عميقة" مضمنة في JavaScript، الذي يقارن الأشياء بالمحتويات ، ولكن من الممكن كتابتها بنفسك \(سيكون هذا أحد التدريبات في نهاية هذا الفصل\).
...
لاحظ أن الكائن المضاف إلى المجلة يبدو غير عادي بعض الشيء.  بدلاً من كتابة الخاصية   `events: events` ، هذا اختصار يعني نفس الشيء - إذا لم يكن  للخاصية في تدوين القوس متبوعًا بقيمة ، فسيتم أخذ قيمتها من الربط الذي يحمل نفس الاسم.
...
عندما تتم كتابة حلقة for بطريقة تُكتب الكلمة الرئيسية بعد تعريف متغير ، فإنها تمسح القيمة التي تأتي بعد. الكلمة الأساسية مكتوبة ، مع تمرير القيمة التي تليها. تستخدم هذه الحلقة ليس فقط للمصفوفات ولكن أيضًا للسلاسل وبعض هياكل البيانات الأخرى. سوف نتعلم كيف يعمل في [الفصل السادس](https://alihhmmre-a.gitbook.io/alijavascript/6).
